...
Simple API for accessing smb servers
...
class SMB:
   """
   Creates a remote connection to an SMB/CIFS server
   """
   ...
   def __connect(self, serverpath, username=None, password=None, localpath=None):
      ...
      if sys.platform == "win32":
         ...
         if username:
            ...
            if password:
               username += ":%s" % (password)
            serverpath = "//%s@%s"  % (username, serverpath)
         cmd = "/sbin/mount_smbfs \"%s\" %s" % (serverpath, localpath)
...
if __name__ == "__main__":
   smbmount = SMB("\\\\hermes\\cs", "ADOBENET\\esbuilder", "es45!44")
   print smbmount.getLocalPath()
   smbmount.close()
